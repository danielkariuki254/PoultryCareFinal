...
const CreateBirds = ({
  ...
  useEffect(() => {
    if (showModal && mortalitId) {
      axiosInstance
        .get(`/mortalit/${mortalitId}`)
        .then((res) => {
  ...
  const onSubmit = (e) => {
    ...
    if (validateForm()) {
      ...
      axiosInstance
        .put(`/mortalit/update/${mortalitId}`, data)
        .then((res) => {
          if (res.data.success) {
            setWorker("");
            setDate("");
  ...
  return (
    <Modal show={showModal} onHide={handleCloseModal}>
      ...
      <Modal.Title className="text-center ">Edit Birds Died</Modal.Title>
      <Modal.Body>
        <form>
